...
> **Vérification d'honnêteté - ce qui fonctionne réellement aujourd'hui :** la logique métier centrale - la cinématique Parol6 (`kinematics/Parol6Kinematics.kt`), le contrat d'état de l'arbre de configuration (`model/HydraState.kt`), l'envoi de commandes atomiques et le relais vocal (`viewmodel/RobotViewModel.kt`), la reconnexion WebSocket (`network/HydraWebSocket.kt`), le parsing des frames MJPEG (`ui/MjpegPlayer.kt`), le canal de mise à jour interne (`update/GitHubReleaseUpdater.kt`, `ReleaseMetadataParser.kt`, `SemanticVersion.kt`), et la gestion des requêtes du relais vocal Wear (`wear/BoundedRequestScope.kt`, `WatchCompanionProtocol.kt`) sont réels et couverts par 56 tests unitaires JVM/Robolectric qui passent (`./gradlew testDebugUnitTest`). Ce que ces tests ne peuvent structurellement pas atteindre - et ce que cet environnement de développement uniquement Windows n'a pas d'appareil Android physique pour vérifier - c'est le vrai `EncryptedSharedPreferences` adossé au Keystore de `network/AuthPrefs.kt`, l'invite empreinte/visage d'`androidx.biometric`, le transport Bluetooth GATT de `network/HydraBleClient.kt`, et le chemin `ImageDecoder` matériel en API 28+ dans `MjpegStreamParser` : du code réel et compilé qui n'a pas été revérifié sur du matériel depuis son écriture. Le lancement/login de base *a* été confirmé en direct sur un appareil réel pour des versions antérieures (voir les entrées `v0.3.2`/`v0.3.3` de `CHANGELOG.md`), mais cela précède le Bluetooth, la biométrie et le chemin de décodage matériel. Voir `CHANGELOG.md` pour ce qui a exactement été livré, y compris ses propres notes par entrée sur ce qui reste non vérifié sur matériel.

## 🏗️ Ce qui est implémenté

- **Contrôle d'Accès et Biométrie** (`ui/LoginScreen.kt`, `util/BiometricHelper.kt`) - Système de connexion professionnel avec prise en charge de l'**empreinte digitale et du déverrouillage facial** (`androidx.biometric`), plus des champs IP/port directement sur le même écran pour pouvoir cibler un serveur sans passer d'abord par les Réglages. Comprend une fonction "Se souvenir de moi", un mécanisme de **déconnexion** sécurisé, et est entièrement localisé en **7 langues**. Le nom d'utilisateur/mot de passe/jeton mis en cache (`network/AuthPrefs.kt`) résident dans des **EncryptedSharedPreferences** adossées au Keystore (AES256-GCM), pas en texte clair - un serveur de développement (`NODE_ENV` non défini) initialise un compte `admin`/`admin` par défaut au premier démarrage ; un serveur de production (`NODE_ENV=production`) exige à la place `HYDRA_UMC_BOOTSTRAP_ADMIN_USERNAME`/`HYDRA_UMC_BOOTSTRAP_ADMIN_PASSWORD` et ne retombe jamais silencieusement sur `admin`/`admin` - voir le README de HYDRA-UMC-SERVER pour la configuration complète de production. Des comptes supplémentaires à privilège réduit de type **operator** restent créables côté serveur depuis Config > Users dans les deux cas.
...
- **Synchronisation des Commandes Atomiques** (le propre `sendAtomicCommand()` de `viewmodel/RobotViewModel.kt`) - Chaque écriture (enable/disable/play/pause/stop/jog/jogStep/valve/pump/speed/vision/tool) envoie une petite commande atomique concernant un seul robot au lieu de l'arbre de configuration entier - le serveur calcule quels robots combinés sont également affectés, persiste sur disque, et diffuse de lui-même à tous les autres clients connectés. Enable/Disable se propage aux propres frères `combinedWith` d'un robot de la même façon que le font Play/Pause/Stop, puisqu'ils partagent tous le même calcul de robots affectés.
...
- **Canal de Mise à Jour In-App** (`update/GitHubReleaseUpdater.kt`, `update/ReleaseMetadataParser.kt`, `update/SemanticVersion.kt`) - Vérifie la dernière GitHub Release stable au lancement et depuis **Paramètres → Mises à jour** ; télécharge uniquement l'actif exact `HYDRA-UMC-ANDROID-CONTROL-release.apk` d'un tag non-brouillon et non-prérelease, sans jamais l'installer automatiquement - le propre installateur de paquets d'Android demande le consentement final. Contrat complet : [`docs/GITHUB_RELEASE_UPDATES.md`](docs/GITHUB_RELEASE_UPDATES.md).
- **Compagnon Wear OS Jumelé et Relais Vocal** (`wear/WatchVoiceRelayService.kt`, `wear/WatchCompanionProtocol.kt`) - Relaie un tour de parole déjà reconnu et initié par l'utilisateur depuis l'application jumelée [HYDRA-UMC-WATCH](https://github.com/JuanenRac/HYDRA-UMC-WATCH) via la propre session Server authentifiée de cette application vers `HYDRA-UMC-VOICE-UI`, renvoyant une réponse typée à la montre ; ni ce relais ni la carte de statut système de la montre n'émettent jamais de commande robot ni ne touchent `HydraState` directement - une réponse liée au mouvement est explicitement marquée `requiresConfirmation` et doit passer par une UI de contrôle principale. Contrat complet : [`docs/WATCH_VOICE_RELAY.md`](docs/WATCH_VOICE_RELAY.md).
- **Assistant Vocal Intégré** (`ui/VoiceAssistantDialog.kt`) - Une icône de micro dans la barre supérieure ouvre une boîte de dialogue pour taper une question/un ordre, ou utiliser la reconnaissance vocale propre à ce téléphone (`RecognizerIntent`, le même schéma réel que le bouton vocal de la montre). Envoie la transcription reconnue via la même route `POST /api/voice/turn` que le relais de la montre, mais comme trafic ordinaire de ce téléphone (`HydraApiClient.postVoiceTurn()`) plutôt qu'un relais - ainsi Config > Accès Distant peut autoriser/bloquer ce bouton indépendamment de la montre jumelée. N'envoie jamais l'audio, et une réponse n'émet jamais de commande robot directement.
...
Ce dépôt suit une politique à l'échelle de l'écosystème : le numéro de version augmente automatiquement à **chaque build réel**, sans modification manuelle de `versionName`/`versionCode` dans `app/build.gradle.kts`. `app/version.properties` conserve les valeurs actuelles de `versionMajor`/`versionMinor`/`versionPatch`/`versionCode` ; `build-android.bat`/`.sh` exécutent `bump_manifest_version.py` (version native + manifeste `hydra-umc.project.json` + `CHANGELOG.md`, tenus synchronisés) et `bump_version_code.py` (le `versionCode` Android, séparé et toujours monotone) *avant* d'invoquer Gradle. `app/build.gradle.kts` lui-même ne fait plus que *lire* `version.properties` au moment de la configuration - il l'écrivait aussi auparavant sur n'importe quelle tâche Gradle réelle, mais une simple compilation de vérification `compileDebugKotlin` ainsi lancée faisait silencieusement avancer `versionPatch`/`versionCode` sans que le manifeste ne bouge jamais (un vrai bug de désynchronisation, trouvé en direct et corrigé - voir `CHANGELOG.md`), donc les deux scripts d'incrémentation sont aujourd'hui la seule véritable source d'un bump. Passer par `build-android.bat`/`.sh` (ou les scripts de release équivalents) produit toujours une version strictement supérieure à la précédente :
...
Un simple `./gradlew assembleDebug` (la voie « Compilation manuelle » ci-dessus, sans exécuter d'abord de script d'incrémentation) réutilise ce que contenait déjà `version.properties` - cela n'incrémente rien tout seul. La version en cours d'exécution est visible en direct dans la boîte de dialogue **À propos** (`BuildConfig.VERSION_NAME`, qui lit le dernier `versionName` que contenait `version.properties`). Voir [CHANGELOG.md](CHANGELOG.md) pour l'historique des versions.
...
│       │   │   ├── VoiceAssistantDialog.kt # Bouton micro/texte intégré vers HYDRA-UMC-VOICE-UI
│       │   │   └── theme/
...
│       │   ├── wear/
│       │   │   ├── BoundedRequestScope.kt     # Portee des taches en cours par requestId ; une tentative remplacee est annulee, cancelAll() a la destruction du service
