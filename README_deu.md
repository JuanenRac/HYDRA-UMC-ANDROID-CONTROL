...
> **Ehrlichkeitscheck - was heute wirklich läuft:** die zentrale Geschäftslogik - Parol6-Kinematik (`kinematics/Parol6Kinematics.kt`), der Settings-Baum-Zustandsvertrag (`model/HydraState.kt`), atomare Befehlszustellung und das Sprach-Relay (`viewmodel/RobotViewModel.kt`), WebSocket-Reconnect-Handling (`network/HydraWebSocket.kt`), MJPEG-Frame-Parsing (`ui/MjpegPlayer.kt`), der In-App-Update-Kanal (`update/GitHubReleaseUpdater.kt`, `ReleaseMetadataParser.kt`, `SemanticVersion.kt`) und die Wear-Sprach-Relay-Anfrageverwaltung (`wear/BoundedRequestScope.kt`, `WatchCompanionProtocol.kt`) sind real und durch 59 bestehende JVM/Robolectric-Unit-Tests abgedeckt (`./gradlew testDebugUnitTest`). Was diese Tests strukturell nicht erreichen können - und was diese reine Windows-Entwicklungsumgebung mangels physischem Android-Gerät nicht verifizieren kann - ist das echte Keystore-gestützte `EncryptedSharedPreferences` von `network/AuthPrefs.kt`, der `androidx.biometric`-Fingerabdruck-/Gesichts-Prompt, der Bluetooth-GATT-Transport von `network/HydraBleClient.kt`, und der API-28+-Hardware-`ImageDecoder`-Pfad in `MjpegStreamParser`: echter, kompilierter Code, der seit seiner Erstellung nicht erneut auf echter Hardware verifiziert wurde. Grundlegendes Starten/Login *wurde* für frühere Releases live auf einem echten Gerät bestätigt (siehe die Einträge `v0.3.2`/`v0.3.3` in `CHANGELOG.md`), das liegt aber vor Bluetooth, Biometrie und dem Hardware-Decode-Pfad. Siehe `CHANGELOG.md` für genau das, was bereits ausgeliefert wurde, samt eigener Hinweise pro Eintrag, was hardwareseitig noch unverifiziert bleibt.

## 🏗️ Was implementiert ist

- **Zugriffskontrolle & Biometrie** (`ui/LoginScreen.kt`, `util/BiometricHelper.kt`) - Professionelles Login-System mit Unterstützung für **Fingerabdruck und Face Unlock** (`androidx.biometric`), plus IP-/Port-Felder direkt auf demselben Bildschirm, damit ein Server angesteuert werden kann, ohne zuerst einen separaten Umweg über die Einstellungen zu nehmen. Enthält eine "Angemeldet bleiben"-Funktion, einen sicheren **Logout**-Mechanismus und ist vollständig in **7 Sprachen** lokalisiert. Der zwischengespeicherte Benutzername/das Passwort/Token (`network/AuthPrefs.kt`) liegen in Keystore-gestützten **EncryptedSharedPreferences** (AES256-GCM), nicht im Klartext - ein Entwicklungsserver (`NODE_ENV` nicht gesetzt) legt beim ersten Start ein Standardkonto `admin`/`admin` an; ein Produktionsserver (`NODE_ENV=production`) verlangt stattdessen `HYDRA_UMC_BOOTSTRAP_ADMIN_USERNAME`/`HYDRA_UMC_BOOTSTRAP_ADMIN_PASSWORD` und greift nie stillschweigend auf `admin`/`admin` zurück - siehe das eigene README von HYDRA-UMC-SERVER für die vollständige Produktionskonfiguration. Zusätzliche, weniger privilegierte **operator**-Konten lassen sich in beiden Fällen serverseitig unter Config > Users erstellen.
...
- **3D-Ansicht** (`ui/ThreeDScreen.kt`) - Bettet das eigene Echtzeit-3D-Viewport von HYDRA-UMC STUDIO in eine WebView ein (`?hideUI=true&robotId=&token=`), statt eines nativen Renderers. Der WebView-Ansatz erhält die echte, aktuell ausgelieferte STUDIO-3D-Szene (jedes echte Roboter-Mesh/jede echte Kinematik) kostenlos, statt sie nativ neu zu implementieren - der Kompromiss ist der WebView-Rendering-Overhead, nicht batterieoptimal, aber heute funktional vollständig.
- **Echtzeit-Oktal-Vision** (`ui/CameraScreen.kt`, `ui/MjpegPlayer.kt`) - **Nativer MJPEG-Streamer** auf Industrieniveau. Verfügt über einen autonomen Hintergrund-Parser und einen Canvas-basierten Renderer für Videotelemetrie mit null Latenz, einen klaren "Kamera deaktiviert"-Zustand (statt eines stillschweigend leeren Feeds), wenn das Vision-System eines Roboters ausgeschaltet ist, sowie einen Schalter, um die Kamera eines Roboters direkt vom Server aus ein-/auszuschalten. Unterstützt automatische **Picture-in-Picture (PIP)**-Overlays im Bildschirm für manuelle Steuerung, die über die Kamerakonfiguration des Servers bestimmten Robotern zugeordnet werden. Ab API 28 wird jedes Bild über den eigenen hardwarebeschleunigten `ImageDecoder`-Pfad der Plattform dekodiert (ein GPU-residentes Bitmap, ohne die CPU-seitige Dekodierung/Übertragung), mit einem automatischen, selbstheilenden Fallback auf die ursprüngliche CPU-Dekodierung.
...
- **Sprachassistent in der App** (`ui/VoiceAssistantDialog.kt`) - Ein Mikrofonsymbol in der oberen Leiste öffnet einen Dialog zum Eintippen einer Frage/eines Befehls oder zur Nutzung der eigenen Spracherkennung dieses Telefons (`RecognizerIntent`, dasselbe reale Muster, das die Sprachtaste der Uhr bereits verwendet). Sendet die erkannte Transkription über dieselbe `POST /api/voice/turn`-Route wie das Uhr-Relay, jedoch als gewöhnlichen Datenverkehr dieses Telefons (`HydraApiClient.postVoiceTurn()`) statt als Relay - so kann Config > Fernzugriff diese Taste unabhängig von der gekoppelten Uhr erlauben/blockieren. Lädt niemals Audio hoch, und eine Antwort löst nie direkt einen Roboterbefehl aus.
...
Dieses Repository folgt einer Richtlinie für das gesamte Ökosystem: Die Version wird bei **jedem echten Build** automatisch erhöht, ohne manuelle Bearbeitung von `versionName`/`versionCode` in `app/build.gradle.kts`. `app/version.properties` speichert die aktuellen Werte von `versionMajor`/`versionMinor`/`versionPatch`/`versionCode`; `build-android.bat`/`.sh` führen `bump_manifest_version.py` (native Version + `hydra-umc.project.json`-Manifest + `CHANGELOG.md`, synchron gehalten) und `bump_version_code.py` (den separaten, stets monoton steigenden Android-`versionCode`) *vor* dem Aufruf von Gradle aus. `app/build.gradle.kts` selbst *liest* `version.properties` jetzt nur noch zur Konfigurationszeit - früher schrieb es die Datei dort auch bei jeder echten Gradle-Aufgabe, aber ein einfacher Verifikations-Build mit `compileDebugKotlin` ließ dabei still `versionPatch`/`versionCode` weiterlaufen, ohne dass sich das Manifest je mitbewegte (ein echter Drift-Bug, live gefunden und behoben - siehe `CHANGELOG.md`), weshalb die beiden Bump-Skripte heute die einzige echte Quelle eines Versionssprungs sind. Der Weg über `build-android.bat`/`.sh` (oder die entsprechenden Release-Skripte) erzeugt immer eine Version, die strikt höher ist als die vorherige:
...
Ein einfaches `./gradlew assembleDebug` (der Weg "Manuelles Erstellen" oben, ohne vorher eines der Bump-Skripte auszuführen) verwendet einfach weiter, was zuletzt in `version.properties` stand - es erhöht dabei von sich aus nichts. Die laufende Version ist live im **Info**-Dialog sichtbar (`BuildConfig.VERSION_NAME`, das den zuletzt in `version.properties` stehenden `versionName` liest). Siehe [CHANGELOG.md](CHANGELOG.md) für die Versionshistorie.
...
│       │   │   ├── VoiceAssistantDialog.kt # Eigene Mikrofon-/Text-Schaltfläche zu HYDRA-UMC-VOICE-UI
│       │   │   └── theme/
