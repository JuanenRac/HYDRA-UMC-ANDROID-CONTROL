...
class HydraApiClient(host: String, port: Int, private val client: OkHttpClient = sharedHttpClient) {
    ...
    /**
     * Sends this phone's own typed/recognised voice turn - the in-app
     * assistant button (VoiceAssistantDialog), not a relay on a paired
     * Watch's behalf. Same wire shape and same Server route as
     * postWatchVoiceTurn() above (Server does not distinguish the two
     * beyond the client-identity header below), reusing WatchVoiceTurn/
     * WatchAssistantReply rather than duplicating an identical contract -
     * their name predates this second caller.
     */
    suspend fun postVoiceTurn(turn: WatchVoiceTurn): WatchAssistantReply = withContext(Dispatchers.IO) {
        val request = Request.Builder()
            .url("$baseUrl/api/voice/turn")
            .header("Authorization", "Bearer ${authToken ?: ""}")
            // Deliberately no "X-Hydra-Client: watch" header here - left to
            // the shared interceptor's own "android" default, so Config >
            // Remote Access > Android gates this call, independently of the
            // separate Watch toggle postWatchVoiceTurn() answers to (see
            // that method's own comment, and server.ts's own
            // remoteAccessAllowed()).
            .post(turn.toJson().toString().toRequestBody(JSON_MEDIA_TYPE))
            .build()
        WatchAssistantReply.fromJson(executeExpectingJson(request))
