...
class HydraWebSocket(
     ...
     * as a full-tree replace, same as before this existed. Placed BEFORE
     * onError (not after) so onError - not this - stays the parameter the
     * existing trailing-lambda call site in RobotViewModel.kt's own
     * setupWebSocket() binds to.
     */
    private val onDelta: ((JSONObject) -> Unit)? = null,
    ...
    private fun openSocket() {
        ...
        // remoteApiVersion=2 declares this connection understands a real targeted
    ...
    private fun handleMessage(text: String) {
        ...
        if (type == "delta" && json.optInt("schema") == 2 && json.has("robotId")) {
            val handler = onDelta
            if (handler != null) {
                mainScope.launch { handler(json) }
                return
            }
            // No onDelta wired up - falls through to the payload check below,
            // which safely no-ops (a schema-2 delta has no `payload` key)
            // rather than crash or apply a corrupted full-tree replace.
        }
