...
- Gradle project (AGP 8.2.0, Kotlin 1.9.22, compileSdk 34, targetSdk 34, minSdk 24) with the real Gradle wrapper checked in (`gradlew`, `gradlew.bat`, `gradle/wrapper/*`), a launcher icon (adaptive on API 26+, vector fallback below), and `android:usesCleartextTraffic="true"` (the server is plain `http`/`ws`, no TLS, by design - see `AndroidManifest.xml`'s own comment).
- `build-android.sh` / `build-android.bat` - one-shot build + `adb install` to a connected device, with pre-flight checks for the Gradle wrapper, the Android SDK location, and the JDK version (AGP 8.2.0 needs JDK 17+ to run Gradle at all).
...
Requires **a JDK 17 specifically** (not older, not "whatever's newest") and the Android SDK. The easiest way to get both at once, correctly matched, is Android Studio:

1. Install [Android Studio](https://developer.android.com/studio).
2. Open it → **Open** → select this repo's root folder, and let the initial Gradle sync finish. This generates `local.properties` (`sdk.dir=...`) automatically and, if `compileSdk 34` isn't installed yet, Android Studio shows an "Install missing platform(s) and sync project" prompt at the top - accept it.
...
- **JDK 17**: install one (e.g. [Eclipse Temurin 17](https://adoptium.net/temurin/releases/?version=17)) and point `JAVA_HOME` at it. Don't grab whatever the latest available JDK is - this project pins Gradle 8.2 (`gradle/wrapper/gradle-wrapper.properties`), and Gradle 8.2 doesn't know how to run on a JDK much newer than ~20 either (see Troubleshooting below); JDK 17 is the version this whole toolchain (AGP 8.2.0 / Gradle 8.2 / Kotlin 1.9.22) is actually validated against.
- **Android SDK**: install the ["command line tools only"](https://developer.android.com/studio) package, set `ANDROID_HOME` to where you extracted it, then `sdkmanager "platform-tools" "platforms;android-34" "build-tools;34.0.0" && sdkmanager --licenses`.
...
| `No matching variant of com.android.tools.build:gradle:8.2.0 was found ... compatible with Java 8` | Active JDK is 8 (or older) | Install/select a JDK 17, point `JAVA_HOME` at it |
| Build fails almost instantly with a bare, unhelpful `* What went wrong: 26.0.2` (or any other version-number-only message), sometimes preceded by `WARNING: A restricted method in java.lang.System has been called` | Active JDK is *too new* - Gradle 8.2 (pinned by this project) doesn't know how to run on a JDK much past ~20; it doesn't fail with a clear "unsupported JDK" message, just a cryptic one like this | Install/select a JDK **17** specifically, not whatever the newest available JDK happens to be - point `JAVA_HOME` at it |
| `:app:checkDebugAarMetadata FAILED` listing many `androidx.*` dependencies that "require... to compile against version 34 or later of the Android APIs" | A dependency bump (Compose BOM, AndroidX libraries) pulled in artifacts compiled against a newer API level than this project's own `compileSdk` | Bump `compileSdk` in `app/build.gradle.kts` to match (currently 34) - independent of `targetSdk`, which doesn't need to move with it. If AGP's own max-recommended `compileSdk` is below what's needed (check the same error message), AGP needs bumping too - AGP 8.2.0 supports `compileSdk 34` while still only requiring Gradle 8.2 (this project's own pinned wrapper version) |
...
| Phone shows "This app was built for an older version of Android" during install | `targetSdk` below the installing device's own enforced floor - Android 14 specifically refuses to install anything with `targetSdk < 23` outright | `targetSdk` is 34 (this project's own current value, matching `compileSdk`) - if it's showing anyway, the installed APK is stale from an earlier build attempt with an older `targetSdk`; uninstall the app from the phone first, then rebuild (`gradlew clean` if a plain rebuild doesn't help) and reinstall rather than upgrading in place |
