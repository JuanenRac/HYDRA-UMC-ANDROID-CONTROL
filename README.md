...
- Gradle project (AGP 8.2.0, Kotlin 1.9.22, compileSdk 34, targetSdk 33, minSdk 24) with the real Gradle wrapper checked in (`gradlew`, `gradlew.bat`, `gradle/wrapper/*`), a launcher icon (adaptive on API 26+, vector fallback below), and `android:usesCleartextTraffic="true"` (the server is plain `http`/`ws`, no TLS, by design - see `AndroidManifest.xml`'s own comment). compileSdk is 34 (not 33) because the Compose/AndroidX dependency tree pulled in by `compose-bom 2024.02.00` requires it at compile time, independent of `targetSdk` - see `app/build.gradle.kts`'s own comments.
