...
2. Open it → **Open** → select this repo's root folder, and let the initial Gradle sync finish. This generates `local.properties` (`sdk.dir=...`) automatically and, if `compileSdk 33` isn't installed yet, Android Studio shows an "Install missing platform(s) and sync project" prompt at the top - accept it.
...
| `No matching variant of com.android.tools.build:gradle:8.1.0 was found ... compatible with Java 8` | Active JDK is 8 (or older) | Install/select a JDK 17, point `JAVA_HOME` at it |
