...
> **Honesty check - what actually runs today:** the core business logic - Parol6 kinematics (`kinematics/Parol6Kinematics.kt`), the settings-tree state contract (`model/HydraState.kt`), atomic command dispatch and the voice-turn relay (`viewmodel/RobotViewModel.kt`), WebSocket reconnect handling (`network/HydraWebSocket.kt`), MJPEG frame parsing (`ui/MjpegPlayer.kt`), the in-app update channel (`update/GitHubReleaseUpdater.kt`, `ReleaseMetadataParser.kt`, `SemanticVersion.kt`), and the Wear voice-relay request bookkeeping (`wear/BoundedRequestScope.kt`, `WatchCompanionProtocol.kt`) are real and covered by 59 passing JVM/Robolectric unit tests (`./gradlew testDebugUnitTest`). What those tests structurally can't reach - and what this Windows-only dev environment has no physical Android device to verify - is `network/AuthPrefs.kt`'s real Keystore-backed `EncryptedSharedPreferences`, the `androidx.biometric` fingerprint/face prompt, `network/HydraBleClient.kt`'s Bluetooth GATT transport, and the API 28+ hardware `ImageDecoder` path in `MjpegStreamParser`: real, compiled code that hasn't been re-verified on hardware since it was written. Basic launch/login *was* confirmed live on a real device for earlier releases (see `CHANGELOG.md`'s `v0.3.2`/`v0.3.3` entries), but that predates Bluetooth, biometrics, and the hardware-decode path. See `CHANGELOG.md` for exactly what has shipped, including its own per-entry notes on what remains hardware-unverified.

## 🏗️ What's implemented

- **Access Control & Biometrics** (`ui/LoginScreen.kt`, `util/BiometricHelper.kt`) - Professional login system with **Fingerprint and Face Unlock** support (`androidx.biometric`), plus IP/port fields right on the same screen so a server can be pointed at without a separate trip through Settings first. Includes "Remember me" functionality, a secure **Logout** mechanism, and fully localized in **7 languages**. The cached username/password/token (`network/AuthPrefs.kt`) live in Keystore-backed **EncryptedSharedPreferences** (AES256-GCM), not plaintext - a development server (`NODE_ENV` unset) seeds a default `admin`/`admin` account on first start; a production server (`NODE_ENV=production`) requires `HYDRA_UMC_BOOTSTRAP_ADMIN_USERNAME`/`HYDRA_UMC_BOOTSTRAP_ADMIN_PASSWORD` instead and never falls back to `admin`/`admin` silently - see HYDRA-UMC-SERVER's own README for the full production setup. Additional lower-privilege **operator** accounts are creatable server-side from Config > Users either way.
...
- **3D View** (`ui/ThreeDScreen.kt`) - Embeds HYDRA-UMC STUDIO's own real-time 3D viewport in a WebView (`?hideUI=true&robotId=&token=`), rather than a native renderer. The WebView approach gets the real, currently-shipping STUDIO 3D scene (every real robot mesh/kinematics) for free instead of reimplementing it natively - the tradeoff is WebView rendering overhead, not battery-optimal, but functionally complete today.
- **Real-time Octal Vision** (`ui/CameraScreen.kt`, `ui/MjpegPlayer.kt`) - Industrial-grade **Native MJPEG Streamer**. Features an autonomous background parser and Canvas-based renderer for zero-latency video telemetry, a clear "Camera Disabled" state (instead of a silently blank feed) when a robot's vision system is off, and a switch to turn a robot's camera on/off directly from the server. Supports automatic **Picture-in-Picture (PIP)** overlays in the manual control screen, mapped to specific robots via the server's camera configuration. On API 28+, each frame decodes through the platform's own hardware-accelerated `ImageDecoder` path (a GPU-resident bitmap, skipping the CPU-side decode/upload) with an automatic, self-healing fallback to the original CPU decode.
...
- **In-App Voice Assistant** (`ui/VoiceAssistantDialog.kt`) - A mic icon in the top bar opens a dialog to type a question/order, or use this phone's own on-device speech recognition (`RecognizerIntent`, same real pattern as the Watch's own voice button). Sends the recognized transcript through the same `POST /api/voice/turn` route the Watch relay uses, but as this phone's own ordinary traffic (`HydraApiClient.postVoiceTurn()`) rather than a relay - so Config > Remote Access can allow/block this button independently of the paired Watch. Never uploads audio, and a reply never invokes a robot command directly.
...
This repo follows an ecosystem-wide policy: the version bumps automatically on **every real build**, no manual editing of `app/build.gradle.kts`'s `versionName`/`versionCode`. `app/version.properties` holds the current `versionMajor`/`versionMinor`/`versionPatch`/`versionCode`; `build-android.bat`/`.sh` run `bump_manifest_version.py` (native version + `hydra-umc.project.json` manifest + `CHANGELOG.md`, kept in sync) and `bump_version_code.py` (the separate, always-monotonic Android `versionCode`) *before* invoking Gradle. `app/build.gradle.kts` itself only ever *reads* `version.properties` at configuration time now - it used to write it there too on any real Gradle task, but a plain verification `compileDebugKotlin` run that way silently advanced `versionPatch`/`versionCode` with the manifest never moving (a real drift bug, found live and fixed - see `CHANGELOG.md`), so the two bump scripts are the only real source of a bump today. Going through `build-android.bat`/`.sh` (or the equivalent release scripts) always produces a version strictly newer than the last one:
...
A plain `./gradlew assembleDebug` (the "Manual Build" path above, with neither bump script run first) reuses whatever `version.properties` last had - it does not bump anything on its own. The running version is visible live in the **About** dialog (`BuildConfig.VERSION_NAME`, reading whichever `versionName` `version.properties` last held). See [CHANGELOG.md](CHANGELOG.md) for the version history.
...
│       │   ├── wear/
│       │   │   ├── BoundedRequestScope.kt     # Per-requestId in-flight job scope; a superseded attempt is cancelled, cancelAll() on service destroy
