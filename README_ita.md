...
> **Verifica di onestà - cosa funziona davvero oggi:** la logica di business principale - la cinematica Parol6 (`kinematics/Parol6Kinematics.kt`), il contratto di stato dell'albero di settings (`model/HydraState.kt`), l'invio di comandi atomici e il relay vocale (`viewmodel/RobotViewModel.kt`), la riconnessione WebSocket (`network/HydraWebSocket.kt`), il parsing dei frame MJPEG (`ui/MjpegPlayer.kt`), il canale di aggiornamento interno (`update/GitHubReleaseUpdater.kt`, `ReleaseMetadataParser.kt`, `SemanticVersion.kt`), e la gestione delle richieste del relay vocale Wear (`wear/BoundedRequestScope.kt`, `WatchCompanionProtocol.kt`) sono reali e coperti da 56 test unitari JVM/Robolectric che passano (`./gradlew testDebugUnitTest`). Ciò che questi test strutturalmente non possono raggiungere - e ciò che questo ambiente di sviluppo solo-Windows non ha un dispositivo Android fisico per verificare - è il vero `EncryptedSharedPreferences` basato su Keystore di `network/AuthPrefs.kt`, il prompt impronta/volto di `androidx.biometric`, il trasporto Bluetooth GATT di `network/HydraBleClient.kt`, e il percorso `ImageDecoder` hardware in API 28+ dentro `MjpegStreamParser`: codice reale e compilato che non è stato riverificato su hardware da quando è stato scritto. L'avvio/login di base *è stato* confermato dal vivo su un dispositivo reale per release precedenti (vedi le voci `v0.3.2`/`v0.3.3` di `CHANGELOG.md`), ma questo precede Bluetooth, biometria e il percorso di decodifica hardware. Vedi `CHANGELOG.md` per cosa è stato esattamente consegnato, incluse le sue stesse note per voce su cosa resta non verificato su hardware.

## 🏗️ Cosa è implementato

- **Controllo Accessi e Biometria** (`ui/LoginScreen.kt`, `util/BiometricHelper.kt`) - Sistema di login professionale con supporto per **Impronta digitale e Sblocco con il Volto** (`androidx.biometric`), più campi IP/porta direttamente nella stessa schermata così un server può essere selezionato senza passare prima dalle Impostazioni. Include la funzione "Ricordami", un meccanismo di **Logout** sicuro, ed è completamente localizzato in **7 lingue**. Nome utente/password/token in cache (`network/AuthPrefs.kt`) risiedono in **EncryptedSharedPreferences** basate su Keystore (AES256-GCM), non in chiaro - un server di sviluppo (`NODE_ENV` non impostato) crea un account predefinito `admin`/`admin` al primo avvio; un server di produzione (`NODE_ENV=production`) richiede invece `HYDRA_UMC_BOOTSTRAP_ADMIN_USERNAME`/`HYDRA_UMC_BOOTSTRAP_ADMIN_PASSWORD` e non ricade mai silenziosamente su `admin`/`admin` - vedere il README di HYDRA-UMC-SERVER per la configurazione completa di produzione. Account aggiuntivi a privilegio inferiore di tipo **operator** restano creabili lato server da Config > Users in entrambi i casi.
...
- **Sincronizzazione dei Comandi Atomici** (il `sendAtomicCommand()` proprio di `viewmodel/RobotViewModel.kt`) - Ogni scrittura (enable/disable/play/pause/stop/jog/jogStep/valve/pump/speed/vision/tool) invia un piccolo comando atomico per un singolo robot invece dell'intero albero delle impostazioni - il server calcola quali robot combinati sono anch'essi coinvolti, persiste su disco, e trasmette autonomamente a ogni altro client connesso. Enable/Disable si propaga ai fratelli `combinedWith` di un robot nello stesso modo in cui lo fanno Play/Pause/Stop, poiché tutti condividono lo stesso calcolo dei robot coinvolti.
...
- **Canale di Aggiornamento In-App** (`update/GitHubReleaseUpdater.kt`, `update/ReleaseMetadataParser.kt`, `update/SemanticVersion.kt`) - Controlla l'ultima GitHub Release stabile all'avvio e da **Impostazioni → Aggiornamenti**; scarica solo l'asset esatto `HYDRA-UMC-ANDROID-CONTROL-release.apk` da un tag non-bozza e non-prerelease, senza mai installare automaticamente - il package installer di Android chiede il consenso finale. Contratto completo: [`docs/GITHUB_RELEASE_UPDATES.md`](docs/GITHUB_RELEASE_UPDATES.md).
- **Companion Wear OS Abbinato e Relay Vocale** (`wear/WatchVoiceRelayService.kt`, `wear/WatchCompanionProtocol.kt`) - Inoltra un turno vocale già riconosciuto e avviato dall'utente dall'app abbinata [HYDRA-UMC-WATCH](https://github.com/JuanenRac/HYDRA-UMC-WATCH) tramite la propria sessione Server autenticata di questa app verso `HYDRA-UMC-VOICE-UI`, restituendo una risposta tipizzata all'orologio; né questo relay né la card di stato del sistema dell'orologio emettono mai un comando robot né toccano `HydraState` direttamente - una risposta relativa al movimento è esplicitamente marcata `requiresConfirmation` e deve passare attraverso una UI di controllo primaria. Contratto completo: [`docs/WATCH_VOICE_RELAY.md`](docs/WATCH_VOICE_RELAY.md).
- **Assistente Vocale in App** (`ui/VoiceAssistantDialog.kt`) - Un'icona del microfono nella barra superiore apre una finestra di dialogo per digitare una domanda/un ordine, o usare il riconoscimento vocale proprio di questo telefono (`RecognizerIntent`, lo stesso schema reale già usato dal pulsante vocale dell'orologio). Invia la trascrizione riconosciuta tramite la stessa rotta `POST /api/voice/turn` usata dal relay dell'orologio, ma come traffico ordinario di questo telefono (`HydraApiClient.postVoiceTurn()`) anziché un relay - così Config > Accesso Remoto può consentire/bloccare questo pulsante indipendentemente dall'orologio abbinato. Non carica mai l'audio, e una risposta non emette mai un comando robot direttamente.
...
Questo repository segue una politica a livello di ecosistema: la versione aumenta automaticamente ad **ogni build reale**, senza modifiche manuali a `versionName`/`versionCode` in `app/build.gradle.kts`. `app/version.properties` conserva i valori attuali di `versionMajor`/`versionMinor`/`versionPatch`/`versionCode`; `build-android.bat`/`.sh` eseguono `bump_manifest_version.py` (versione nativa + manifest `hydra-umc.project.json` + `CHANGELOG.md`, tenuti sincronizzati) e `bump_version_code.py` (il `versionCode` Android, separato e sempre monotono) *prima* di invocare Gradle. `app/build.gradle.kts` di per sé ora si limita a *leggere* `version.properties` in fase di configurazione - in passato lo scriveva anche lui su qualsiasi task Gradle reale, ma una semplice compilazione di verifica `compileDebugKotlin` eseguita così faceva avanzare silenziosamente `versionPatch`/`versionCode` senza che il manifest si muovesse mai (un vero bug di disallineamento, trovato dal vivo e corretto - vedi `CHANGELOG.md`), quindi oggi i due script di incremento sono l'unica vera fonte di un bump. Passare per `build-android.bat`/`.sh` (o gli script di release equivalenti) produce sempre una versione strettamente superiore all'ultima:
...
Un semplice `./gradlew assembleDebug` (il percorso "Compilazione Manuale" sopra, senza eseguire prima alcuno script di incremento) riutilizza quanto già presente in `version.properties` - non incrementa nulla da solo. La versione in esecuzione è visibile in tempo reale nella finestra **Informazioni** (`BuildConfig.VERSION_NAME`, che legge l'ultimo `versionName` presente in `version.properties`). Vedi [CHANGELOG.md](CHANGELOG.md) per la cronologia delle versioni.
...
│       │   │   ├── VoiceAssistantDialog.kt # Pulsante microfono/testo in-app verso HYDRA-UMC-VOICE-UI
│       │   │   └── theme/
...
│       │   ├── wear/
│       │   │   ├── BoundedRequestScope.kt     # Ambito dei job in corso per requestId; un tentativo sostituito viene annullato, cancelAll() alla distruzione del servizio
